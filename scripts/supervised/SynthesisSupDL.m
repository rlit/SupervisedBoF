function [D,M,P] = SynthesisSupDL(X,tripletT,tripletV,params,D,M,P)
%
...
for iter = 1:params.max_iter
    ...
    isSel = randsample(nTrain,params.batchSize);
    Tsel  = tripletT(isSel,:);
    ...
    if params.isUpdate_P
        ...
        [Pnew,stepP,iterP] = ArmijoStep(P,dP,lossFunP,step,params.armijo_params,lossP);

        Pnew = FixMatrix(Pnew,2,params.isDiagonal_P);
    else
        Pnew = P;
        stepP=inf;
        iterP=0;
    ...
    if params.isUpdate_M
        ...
        [Mnew,stepM,iterM] = ArmijoStep(M,dM,lossFunM,step,params.armijo_params,lossM);

        Mnew = FixMatrix(Mnew,2,params.isDiagonal_M);
    else
        Mnew = M;
        stepM=inf;
        iterM=0;
    ...
    %lossT = mean(GetLassoGrad(Tsel,X,M,D,P,params));

    lossTnew = mean(GetGradients(Tsel,X,Mnew,D,Pnew,params));
    lossT = lossTnew;
    M=Mnew;
    P=Pnew;
    % iter > 1 && lossTnew>lossT
...
% isDiag = isequal( matIn~=0 ,eye(size(matIn),'logical'));

if isDiag
    %assert(isequal( matIn~=0 ,eye(size(matIn),'logical')))
